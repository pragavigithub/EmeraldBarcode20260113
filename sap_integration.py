...
class SAPIntegration:
    ...
    def get_serial_current_location(self, serial_number):
        ...
        try:
            # First try the custom SQL query
            url = f"{self.base_url}/b1s/v1/SQLQueries('get_serial_current_location')/List"
            payload = {
                "ParamList": f"serial_number='{serial_number}'"
            }
            logging.info(f"🔍 Fetching location for serial: {serial_number}")
            response = self.session.post(url, json=payload, timeout=30)
            ...
            if response.status_code == 200:
                ...
                if values:
                    result = values[0]
                    # Fetch bin details if BinAbsEntry is present
                    bin_abs = result.get('BinAbsEntry')
                    bin_details = None
                    if bin_abs:
                        bin_details = self.get_bin_details(bin_abs)
                    ...
                    return {'success': False, 'error': f'Serial number {serial_number} not found or has no quantity'}
            else:
                logging.warning(f"SQL Query failed: {response.status_code} - {response.text}")
                # Try fallback method using standard APIs
                return self._get_serial_location_fallback(serial_number)
                
        except Exception as e:
            logging.error(f"Error with SQL query, trying fallback: {str(e)}")
            # Try fallback method using standard APIs
            return self._get_serial_location_fallback(serial_number)

    def _get_serial_location_fallback(self, serial_number):
        """Fallback method to find serial location using standard SAP B1 APIs"""
        try:
            logging.info(f"🔄 Using fallback method for serial: {serial_number}")
            
            # Search in SerialNumbers API
            url = f"{self.base_url}/b1s/v1/SerialNumbers?$filter=InternalSerialNumber eq '{serial_number}'"
            response = self.session.get(url, timeout=30)
            
            if response.status_code == 200:
                data = response.json()
                serials = data.get('value', [])
                
                if serials:
                    serial_data = serials[0]
                    item_code = serial_data.get('ItemCode', '')
                    
                    # Try to get more details from Items API
                    item_url = f"{self.base_url}/b1s/v1/Items('{item_code}')"
                    item_response = self.session.get(item_url, timeout=30)
                    ...
                    # Create a mock response similar to the SQL query format
                    result = {
                        'DistNumber': serial_number,
                        'ItemCode': item_code,
                        'itemName': item_name,
                        'WhsCode': serial_data.get('Location', 'Unknown'),
                        'WhsName': f"Warehouse {serial_data.get('Location', 'Unknown')}",
                        'BPLName': 'Default Branch',
                        'BPLid': 1,
                        'BinAbsEntry': None  # Not available in fallback
                    }
                    ...
                    return {'success': False, 'error': f'Serial number {serial_number} not found'}
            else:
                return {'success': False, 'error': f'Failed to search serial numbers: {response.status_code}'}
                
        except Exception as e:
            logging.error(f"Fallback method failed: {str(e)}")
            return {'success': False, 'error': f'All methods failed: {str(e)}'}

    def get_bin_details(self, bin_abs_entry):
        """Get bin location details from SAP B1"""
        if not self.ensure_logged_in():
            return None
        try:
            # Use filter as specified in user requirement
            url = f"{self.base_url}/b1s/v1/BinLocations?$select=AbsEntry,BinCode,Warehouse&$filter=AbsEntry eq {bin_abs_entry}"
            response = self.session.get(url, timeout=30)
            if response.status_code == 200:
                return response.json()
            return None
        except Exception as e:
            logging.error(f"Error fetching bin details: {str(e)}")
            return None

    def create_stock_transfer(self, transfer_data):
        """Create a stock transfer in SAP B1 using complete JSON payload"""
        if not self.ensure_logged_in():
            return {'success': False, 'error': 'SAP B1 connection unavailable'}
        ...
        try:
            ...
            response = self.session.post(url, json=transfer_data, timeout=60)

            if response.status_code in [201, 200]:
                logging.info(f"✅ Stock transfer successful: {response.status_code}")
                return {'success': True, 'data': response.json()}
            else:
                logging.error(f"❌ Failed to create stock transfer: {response.status_code} - {response.text}")
                return {'success': False, 'error': response.text}
        except Exception as e:
            logging.error(f"❌ Error creating stock transfer: {str(e)}")
            return {'success': False, 'error': str(e)}
    ...
    def create_inventory_transfer(self, transfer_document):
        ...
        for index, item in enumerate(transfer_document.items):
            ...
            # 🟦 BATCH-MANAGED ITEM
            if batch_managed:
                batch_map = {}  # { batch_number: cumulative_qty }
                ...
                # Case B: fallback to DB scanned batches
                elif item.batch_number:
                    transfer_states_items = TransferScanState.query.filter_by(
                        transfer_id=transfer_document.id,
                        item_code=item.item_code
                    ).all()

                    for ts in transfer_states_items:
                        if ts.batch_number and ts.qty > 0:
                            batch_map[ts.batch_number] = batch_map.get(
                                ts.batch_number, 0
                            ) + float(ts.qty)

                # Convert map → SAP BatchNumbers format
                line["BatchNumbers"] = [
                    {
                        "BatchNumber": bn,
                        "Quantity": qty
                ...
                # SAP requires batch numbers
                if not line["BatchNumbers"]:
                    raise ValueError(
                        f"❌ BatchNumber REQUIRED for batch-managed item {item.item_code}"
                    )
            ...
            #             transfer_id=transfer_document.id,
            #             item_code=item.item_code
            #         ).all()  # ✅ FETCH ALL BATCHES
            #
            #         for ts in transfer_states_items:
            #             if ts.batch_number and ts.qty > 0:
            #                 line["BatchNumbers"].append({
            #                     "BatchNumber": ts.batch_number,
            #                     "Quantity": float(ts.qty)
            #                 })
            #
            ...
            # -------------------------------------------------------
            # -------------------------------------------------------
            # BIN ALLOCATIONS  ✅ MODIFIED PART ONLY
            # -------------------------------------------------------
            if item.from_bin or item.to_bin:

                line["StockTransferLinesBinAllocations"] = []

                # ✅ CASE 1: BATCH-MANAGED ITEMS → allocate per batch baseline
    ...
    def create_stock_transfer_with_items(self, from_warehouse, to_warehouse, items, comments=''):
        """
